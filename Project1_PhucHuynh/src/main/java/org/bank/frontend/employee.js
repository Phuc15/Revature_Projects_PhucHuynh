...
btn10.addEventListener("click", () => {
    ...
    fetch(url)
        ...
        .then(res1 => {
          if(res1 == 'No bank account with customer id'){
            document.getElementById("err3").innerHTML = "No bank account is found";
            setTimeout(function(){ 
                document.getElementById("err3").innerHTML = "";
            }, 3000);
  ...
  btn4.addEventListener("click", () =>{
    ...
    var num = /^[0-9]+$/;
    if(id == "" || id.length == 0 || !id.match(num)){
        document.getElementById("err4").innerHTML = "Invalid input. Please re-enter";
        setTimeout(function(){ 
  ...
  btn5.addEventListener("click", () =>{
    ...
    var num = /^[0-9]+$/;
    if(id == "" || id.length == 0 || !id.match(num)){
        document.getElementById("err5").innerHTML = "Invalid input. Please re-enter";
        setTimeout(function(){ 
            document.getElementById("err5").innerHTML = "";
        }, 3000);
    }else{
    let url = "http://localhost:8000/employee/bankaccount/transaction-by-accountid/" + id;
    console.log(url);
    fetch(url)
        .then(res => res.json())
        .then(res1 => {
  ...
  btn6.addEventListener("click", () =>{
    ...
    var num = /^[0-9]+$/;
    if(id == "" || id.length == 0 || !id.match(num)){
        document.getElementById("err6").innerHTML = "Invalid input. Please re-enter";
        setTimeout(function(){ 
            document.getElementById("err6").innerHTML = "";
        }, 3000);
    }else{
    let url = "http://localhost:8000/employee/bankaccount/transaction-by-transactionid/" + id;
    console.log(url);
    fetch(url)
        .then(res => res.json())
        .then(res1 => {
  ...
  btn7.addEventListener("click", () =>{
    ...
    if(date == "" || date.length == 0 || !date.match(dateValidation)){
        document.getElementById("err7").innerHTML = "Invalid input. Please re-enter";
        setTimeout(function(){ 
            document.getElementById("err7").innerHTML = "";
        }, 3000);
    }else{
    let url = "http://localhost:8000/employee/bankaccount/transaction-by-date/" +date;
    console.log(url);
    fetch(url)
        .then(res => res.json())
        .then(res1 => {
    ...
    fetch(url)
        ...
        .then(res1 => {
          if(res1 == 'There is no pending customer.'){
            document.getElementById("err8").innerHTML = "No pending customers found";
            setTimeout(function(){ 
                document.getElementById("err8").innerHTML = "";
            }, 3000);
            document.getElementById("banktable-pending-customers").innerHTML = ""
          }else{
            let data = "<table class='table table-bordered table-striped'><thead class='thead-dark'><tr><th>Customer ID</th><th>Name</th><th>Username</th></tr></thead>";
            res1.forEach(element => {
...
btn11.addEventListener("click", () =>{
    ...
    var num = /^[0-9]+$/;
    if(id == "" || id.length == 0 || !id.match(num)){
        document.getElementById("err11").innerHTML = "Invalid input. Please re-enter";
        setTimeout(function(){ 
            document.getElementById("err11").innerHTML = "";
        }, 3000);
    }else{
    let url = "http://localhost:8000/employee/approve-customer-pending/" +id+"/"+ emplyeeUsername;
    console.log(url);
    fetch(url)
        .then(res => res.json())
        .then(res1 => {
            if(res1 == true){
            document.getElementById("err11").innerHTML = "You sucessfully approved this account with id " +id;
            }else{
...
btn12.addEventListener("click", () =>{
    ...
    var num = /^[0-9]+$/;
    if(id == "" || id.length == 0 || !id.match(num)){
        document.getElementById("err12").innerHTML = "Invalid input. Please re-enter";
        setTimeout(function(){ 
    ...
    fetch(url)
        ...
        .then(res1 => {
            if(res1 == true){
            document.getElementById("err12").innerHTML = "You sucessfully reject this account with id " +id;
            }else{
 ...
 btn13.addEventListener("click", () =>{
    ...
    var num = /^[0-9]+$/;
    if(id == "" || id.length == 0 || !id.match(num)){
        document.getElementById("err13").innerHTML = "Invalid input. Please re-enter";
        setTimeout(function(){ 
    ...
    fetch(url)
        ...
        .then(res1 => {
            console.log(res1);
            if(res1.message === 'Id is not found' ||  res1.message === 'Account is still pending to be approved'){
                document.getElementById("err13").innerHTML = "Account is pending or not found. Please try again";
                setTimeout(function(){ 
                    document.getElementById("err13").innerHTML = "";
                }, 3000);
                ...
                let data = "<table class='table table-bordered table-striped'><thead class='thead-dark'><tr><th>Employee Username</th><th>Picture</th></tr></thead>";
                    data = data + "<tr><td>" + res1.status + "</td>"
                    data = data + "<td><img class='rounded' height='200px' width='200px' src='" + "https://yt3.ggpht.com/a/AATXAJz-iBcY8fpw9HE3ngNwiWATqoCSOcvq_6NGTx_U-w=s900-c-k-c0xffffffff-no-rj-mo" + "'/></td></tr>"
                    data = data + "</table>"
