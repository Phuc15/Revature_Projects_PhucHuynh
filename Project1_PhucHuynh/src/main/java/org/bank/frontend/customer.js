...
btn2.addEventListener("click", () => {
  ...
  if (balance == "" || !balance.match(number)) {
    document.getElementById("err1").innerHTML =
      "<p1>invalid input, please re-enter<p1>";
  } else {
    ...
    fetch(url, {
      ...
      .then((res) => {
        if (
          res == "Your account is still pending to be approved. Thank you!" ||
          res == "You already have this account"
        ) {
          document.getElementById("err1").innerHTML = res;
        } else {
          document.getElementById("err1").innerHTML =
            "<p1>YAY! Congrats! You have sucessfully opened a <p1>" +
            accountType +
            " <p1>with $<p1>" +
            balance;
        }
  ...
  if (balance == "" || !balance.match(number)) {
    ...
    fetch(url, {
      ...
      .then((res) => {
        if (res == "successful") {
          document.getElementById("err3").innerHTML =
            "<p1>YAY! Congrats! You have sucessfully deposited <p1>" +
            "<p1>$<p1>" +
            balance +
            " <p1>into<p1> " +
            accountType;
  ...
  if (balance == "" || !balance.match(number)) {
    ...
    fetch(url, {
      ...
      .then((res) => {
        if (res == "successful") {
          document.getElementById("err4").innerHTML =
            "<p1>YAY! Congrats! You have sucessfully withdrew <p1>" +
            "<p1>$<p1>" +
            balance +
            " <p1>from<p1> " +
            accountType;
  ...
  if (balance == "" || !balance.match(number)) {
    ...
    fetch(url, {
      ...
      .then((res) => {
        if (res == "successful") {
          document.getElementById("err5").innerHTML =
            "<p1>YAY! Congrats! You have sucessfully tranfered <p1>" +
            "<p1>$<p1>" +
            balance +
            " <p1>to<p1> " +
            toUser;
...
btn6.addEventListener("click", () => {
  let url =
    "http://localhost:8000/customer/bankaccount/get-pending-transaction/" +
    username;
  //console.log(url);
  fetch(url)
    .then((res) => res.json())
    .then((res1) => {
      if (
        res1 == "There is no pending transaction" ||
        res1 == "No transaction found with the account id"
      ) {
        document.getElementById("err6").innerHTML = res1;
      } else {
        let data =
          "<table class='table table-bordered table-striped'><thead class='thead-dark'><tr><th>Transaction Id</th><th>Pending Amount</th><th>Date</th></tr></thead>";
        res1.forEach((element) => {
...
btn7.addEventListener("click", () => {
  ...
  if (id == "" || id.length == 0 || !id.match(num)) {
    document.getElementById("err7").innerHTML =
      "<P1>ID can not be empty and only contains number [0-9]. Please re-enter again<p1>";
  } else {
    let url =
      "http://localhost:8000/customer/bankaccount/accept-pending-transaction/" +
      id;
    fetch(url)
      .then((res) => res.json())
      .then((res1) => {
        if (res1 == true) {
          document.getElementById("err7").innerHTML =
            "<p1>Pending transaction has been added to your account.<p1>";
        } else {
          document.getElementById("err7").innerHTML =
            "<p1>Transaction ID not found. Please try again!<p1>";
        }
      });
  }
});
...
btn8.addEventListener("click", () => {
  let url =
    "http://localhost:8000/customer/bankaccount/display/previous-transaction/" +
    username;
  //console.log(url);
  fetch(url)
    .then((res) => res.json())
